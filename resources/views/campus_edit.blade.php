...
<body>
    <div class="container">
    <div class="top-nav">
            <a href="/admin" class="dashboard-btn">Back to Dashboard</a>
        </div>
        <h2>Edit Campus</h2>
        ...
        <form action="{{ route('campus.update', $campus->campus_id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="campus">Campus Name</label>
                <input type="text" id="campus" name="campus_name" value="{{ $campus->campus_name }}" required>
            </div>
